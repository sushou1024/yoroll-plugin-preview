...
### 1.1 意图引导的问法（用户选了影游但没给想法时）

用户从创作卡选了「互动影游」却还没说出任何想法时，**禁止抛开放式抽象问题**（如"请用
一句话告诉我：你想创作一个怎样的互动故事？"——用户面对空白问题不知道怎么答）。改为
**先用 1–2 句介绍这个能力能做到什么**（如"我可以从一个想法开始，为你生成完整的互动
影游：剧本、角色、分镜、成片到发布，观众在关键节点做选择，走向不同结局"），**再给出
3–4 个具体的、可直接采纳的示例**，邀请用户选一个或说自己的，例如：

> 我可以从一个想法开始，为你生成完整的互动影游：剧本、角色、分镜、成片到发布，
> 观众在关键节点做选择，走向不同结局。想做个什么样的故事？给你几个方向参考：
> - 仙侠：凡人少女捡到一柄会说话的断剑，每借它出鞘一次，就要替剑中前主偿还一段因果。
> - 克苏鲁：灯塔守夜人连续七夜收到同一封求救信，笔迹是他自己的，落款日期是明天。
> - 赛博朋克：底层义体维修师在报废机体里发现失踪妹妹的记忆芯片，而买家地址指向他修不起的上城区。
> - 治愈喜剧：不会做饭的都市女孩继承了外婆的深夜小面馆，第一位客人就点了一碗只有外婆会做的面。
>
> 选一个我们就开工；也可以直接说你自己的想法，一句话就够。

示例直接使用上方内联的四条或按同样标准现场改写——每条必须**一句话、具体到人物处境**
（有人、有事、有钩子，不是题材名＋氛围词），且整组**覆盖差异明显的调性**。**开场引导
阶段不要读取任何参考文件**（`references/genre-guide.md` 留到用户给出想法后、确定
`genre` 路由值那一步再读），先把问题抛出去，让用户尽快开口。

用户给的想法**很简短也照单全收**：不要追问细节，按本节判定公式先做起来，缺的信息
自己定合理默认值，在汇报里说明选择即可。
